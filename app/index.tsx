...
export default function NewProjectScreen() {
  ...
  const handleCreate = async () => {
    ...
    // Only required docs (non-optional) trigger Phase 0
    const missingRequiredDocs = documents.filter((d) => !d.available && !d.optional);
    const phases = missingRequiredDocs.length > 0
    ...
    try {
      ...
      Alert.alert(
        ...
        missingRequiredDocs.length > 0
          ? `${missingRequiredDocs.length} required document(s) missing — added as Phase 0. Complete document collection before construction begins.`
  ...
  // For status banner — only count required missing docs
  const missingRequiredCount = documents.filter((d) => !d.available && !d.optional).length;
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
          ...
          {step === 0 && (
            <View>
              ...
              <Text style={styles.sectionSubtitle}>
                Mark which documents are available. Required ones missing will be added as Phase 0.
              </Text>

              {/* Status Banner — only for required docs */}
              <View style={[styles.docStatusBanner, allRequiredReady ? styles.docStatusBannerGreen : styles.docStatusBannerAmber]}>
                ...
                <Text style={[styles.docStatusText, allRequiredReady ? styles.docStatusTextGreen : styles.docStatusTextAmber]}>
                  {allRequiredReady
                    ...
                    : `${missingRequiredCount} required document(s) missing — will be added as Phase 0`}
                </Text>
              </View>
              ...
              {requiredDocs.map((doc) => (
                <DocCard
                  ...
                  onRemove={(id) => setDocuments((prev) =>
                    prev.map((d) => d.id === id ? { ...d, fileUri: undefined, fileName: undefined } : d)
                  )}
                />
              ))}
              ...
              {!allRequiredReady && (
                <View style={styles.missingInfoCard}>
                  ...
                  {requiredDocs.filter(d => !d.available).map(d => (
              ...
              <View style={styles.optionalSectionHeader}>
                ...
                <View style={styles.optionalBadge}>
                  <Text style={styles.optionalBadgeText}>Not required to proceed</Text>
                </View>
              </View>
              <Text style={styles.optionalNote}>
                These documents are not mandatory. You can upload them now or skip — they won't affect the project phases.
              </Text>
          ...
          {step === 3 && (
            <Step3
              ...
              totalEstimate={totalEstimate} errors={step3Errors}
            />
          )}
...
function DocCard({ doc, onToggle, onUpload, onRemove, isOptional }: {
  doc: ProjectDocument;
  ...
  return (
    <View style={[styles.docCard, doc.available && styles.docCardAvailable, isOptional && styles.docCardOptional]}>
      ...
      {doc.available && !doc.fileName && (
        <TouchableOpacity style={styles.uploadBtn} onPress={() => onUpload(doc.id)}>
      ...
      {doc.fileName && (
        <View style={styles.uploadedBadge}>
          ...
          <TouchableOpacity onPress={() => onRemove(doc.id)}>
...
function Step1({ customerName, setCustomerName, customerPhone, setCustomerPhone, customerEmail, setCustomerEmail, customerAddress, setCustomerAddress, projectName, setProjectName, siteLocation, setSiteLocation, latitude, longitude, setLatitude, setLongitude, plotArea, setPlotArea, projectType, setProjectType, errors }: any) {
  return (
    <View>
...
const styles = StyleSheet.create({
  ...
  optionalSectionHeader: { flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 20, marginBottom: 6 },
